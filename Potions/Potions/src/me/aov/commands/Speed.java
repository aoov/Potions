...
public class Speed implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (command.getName().equalsIgnoreCase("speedp")) {
			...
			if (args.length == 2 && p.hasPermission("pot.speed.others")) {
				try {
				...
				if ((target.hasPotionEffect(PotionEffectType.SPEED) && target != null)) {
					target.removePotionEffect(PotionEffectType.SPEED);
					p.sendMessage(prefix + speedOff);
				} else if (target != null && !(level == 0 && level >= 0)) {
					target.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 1000000, level - 1), true);
	...
	public static void giveSpeed(Player p) {
		p.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 1000000, getLevel(p)-1, true));
	}
}
