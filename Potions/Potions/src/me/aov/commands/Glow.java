...
public class Glow implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (command.getName().equalsIgnoreCase("glow")) {
			...
			if (args.length == 2 && p.hasPermission("pot.glow.others")) {
				try {
				...
				if ((target.hasPotionEffect(PotionEffectType.GLOWING) && target != null)) {
					target.removePotionEffect(PotionEffectType.GLOWING);
					p.sendMessage(prefix + glowOff);
				} else
				if (target != null && !(level == 0 && level >= 0)) {
					target.addPotionEffect(new PotionEffect(PotionEffectType.GLOWING, 1000000, level - 1), true);
	...
	public static void giveGlow(Player p) {
		p.addPotionEffect(new PotionEffect(PotionEffectType.GLOWING, 1000000, getLevel(p)-1, true));
	}
}
